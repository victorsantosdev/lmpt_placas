...
void Escreve_caracter_LCD(unsigned int Endereco_paralela, unsigned int Endereco_LCD, char caracter_LCD)
{
  ...
  OutPortB(Endereco_paralela + 2, 0xF5); // Nenhum CI selecionado (CS's altos), RS em 1 e R/W em zero
  Sleep(1);
  ...
  LCD = LCD & 0xFB; // Coloca /C3 e C2 em zero

  OutPortB(Endereco_paralela + 2, 0xF1); // Armazerna os valores de RS e R/W no latch
  Sleep(1);
...
void Escreve_instrucao_LCD(unsigned int Endereco_paralela, unsigned int Endereco_LCD, unsigned int instrucao)
{
  ...
  OutPortB(Endereco_paralela + 2, 0xF7); // Nenhum CI selecionado (CS's altos), RS e R/W em zero
  Sleep(1);
  ...
  OutPortB(Endereco_paralela + 2, 0xF3); // Armazerna os valores de RS e R/W no latch
  Sleep(1);
